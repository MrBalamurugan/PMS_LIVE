...
export const HeaderSort = ({ column, sort }: any) => {
...
export const TableRowSelection = ({ selected }: any) => (
  <>
    {selected > 0 && (
      <Chip
        size="small"
...
export const DraggableHeader = ({ children, column, index, reorder }: any) => {
  const theme = useTheme();
  const ref = useRef();
...
export const HidingSelect = ({
  ...
  let visible = allColumns.filter(
    (c: any) => !hiddenColumns.includes(c.id)
  ).length;
          ...
          return (
            <Typography variant="subtitle2">
              ...
              {typeof selectedColumn?.Header === "string"
                ...
                : selectedColumn?.title || "Unnamed Column"}
              )
            </Typography>
          );
        }}
